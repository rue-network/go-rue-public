...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	...
	echo $'grue --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ruestats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*"' >> wallet.sh

RUN \
	sed -i 's/PupprueNetworkID/{{.NetworkID}}/g' dist/js/ruerwallet-master.js && \
	sed -i 's/PupprueNetwork/{{.Network}}/g'     dist/js/ruerwallet-master.js && \
	sed -i 's/PupprueDenom/{{.Denom}}/g'         dist/js/ruerwallet-master.js && \
	sed -i 's/PupprueHost/{{.Host}}/g'           dist/js/ruerwallet-master.js && \
	sed -i 's/PupprueRPCPort/{{.RPCPort}}/g'     dist/js/ruerwallet-master.js
