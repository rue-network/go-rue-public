...
func (w *wizard) deployNode(boot bool) {
	...
	if !boot {
		if w.conf.Genesis.Config.Ruehash != nil {
			// Ruehash based miners only need an ruerbase to mine against
			fmt.Println()
			if infos.ruerbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.ruerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.ruerbase)
				infos.ruerbase = w.readDefaultAddress(common.HexToAddress(infos.ruerbase)).Hex()
			}
