...
// faucet is a Ruer faucet backed by a light client.
package main
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Ruereum protocol")
	statsFlag   = flag.String("ruestats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Ruers to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Ruereum and the faucet")
)

var (
	ruer = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Ruers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Ruereum address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Ruereum light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Ruereum protocol stack
	client *rueclient.Client   // Client connection to the Ruereum chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Ruereum light client protocol
...
// close terminates the Ruereum connection and tears down the faucet.
...
// apiHandler handles requests for Ruer grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, ruer),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Ruereum address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), ruer)
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance = new(big.Int).Div(balance, ruer)
...
// the username, avatar URL and Ruereum address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Ruereum addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Ruereum address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Ruereum address and profile URL.
...
// returning the username, avatar URL and Ruereum address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Ruereum address,
