...
// config contains all the configurations needed by pup.rue that should be saved
// between sessions.
type config struct {
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save pup.rue configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // Ruereum services known to be running on servers
...
// converts it to an Ruereum address. If an empty line is entered, the default
// value is returned.
