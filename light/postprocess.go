...
func GetChtRoot(db ruedb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func GetChtV2Root(db ruedb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	return GetChtRoot(db, (sectionIdx+1)*(ChtFrequency/ChtV1Frequency)-1, sectionHead)
}

// StoreChtRoot writes the CHT root assoctiated to the given section into the database
// Note that sectionIdx is specified according to LES/1 CHT section size
func StoreChtRoot(db ruedb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewChtIndexer(db ruedb.Database, clientMode bool) *core.ChainIndexer {
	cdb := ruedb.NewTable(db, ChtTablePrefix)
	idb := ruedb.NewTable(db, "chtIndex-")
...
const (
	...
	rueBloomBitsSection       = 4096
	rueBloomBitsConfirmations = 256
)
...
func GetBloomTrieRoot(db ruedb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db ruedb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
// NewBloomTrieIndexer creates a BloomTrie chain indexer
func NewBloomTrieIndexer(db ruedb.Database, clientMode bool) *core.ChainIndexer {
	cdb := ruedb.NewTable(db, BloomTrieTablePrefix)
	idb := ruedb.NewTable(db, "bltIndex-")
	...
	if clientMode {
		...
		backend.parentSectionSize = rueBloomBitsSection
		confirmReq = HelperTrieProcessConfirmations
	}
	backend.bloomTrieRatio = BloomTrieFrequency / backend.parentSectionSize
	backend.sectionHeads = make([]common.Hash, backend.bloomTrieRatio)
	return core.NewChainIndexer(db, idb, backend, BloomTrieFrequency, confirmReq-rueBloomBitsConfirmations, time.Millisecond*100, "bloomtrie")
}
