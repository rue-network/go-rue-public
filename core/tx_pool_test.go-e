...
func pricedTransaction(nonce uint64, gaslimit, gasprice *big.Int, key *ecdsa.PrivateKey) *types.Transaction {
	tx, _ := types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(100), gaslimit, gasprice, nil), types.HorizonSigner{}, key)
	return tx
}
...
func deriveSender(tx *types.Transaction) (common.Address, error) {
	return types.Sender(types.HorizonSigner{}, tx)
}
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.Rueer))
		*c.trigger = false
	}
	return stdb, nil
}
...
// state reset and tests whrueer the pending state is in sync with the
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(params.Rueer))
...
func TestTransactionNegativeValue(t *testing.T) {
	...
	tx, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(-1), big.NewInt(100), big.NewInt(1), nil), types.HorizonSigner{}, key)
	from, _ := deriveSender(tx)
	pool.currentState.AddBalance(from, big.NewInt(1))
...
// Tests that the transaction limits are enforced the same way irrelevant whrueer
