...
func GenerateChain(config *params.ChainConfig, parent *types.Block, engine consensus.Engine, db ruedb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	if config == nil {
	...
	genblock := func(i int, parent *types.Block, statedb *state.StateDB) (*types.Block, types.Receipts) {
		...
		// It's nonrueeless ugly to spin up a blockchain here. Get rid of this somehow.
...
// header only chain.
func newCanonical(engine consensus.Engine, n int, full bool) (ruedb.Database, *BlockChain, error) {
	// Initialize a fresh chain with only a genesis block
	gspec := new(Genesis)
	db, _ := ruedb.NewMemDatabase()
	genesis := gspec.MustCommit(db)

	blockchain, _ := NewBlockChain(db, params.AllRuehashProtocolChanges, engine, vm.Config{})
...
func makeHeaderChain(parent *types.Header, n int, engine consensus.Engine, db ruedb.Database, seed int) []*types.Header {
...
func makeBlockChain(parent *types.Block, n int, engine consensus.Engine, db ruedb.Database, seed int) []*types.Block {
