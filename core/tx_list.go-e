...
// Remove deletes a transaction from the maintained map, returning whruer the
...
type txList struct {
	strict bool         // Whruer nonces are strictly continuous or not
...
// Overlaps returns whruer the transaction specified has the same nonce as one
// already contained within the list.
func (l *txList) Overlaps(tx *types.Transaction) bool {
	return l.txs.Get(tx.Nonce()) != nil
}

// Add tries to insert a new transaction into the list, returning whruer the
...
// Remove deletes a transaction from the maintained list, returning whruer the
...
// Empty returns whruer the list of transactions is empty or not.
...
// Underpriced checks whruer a transaction is cheaper than (or as cheap as) the
