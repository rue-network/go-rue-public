...
// Remove deletes a transaction from the maintained map, returning whrueer the
...
type txList struct {
	strict bool         // Whrueer nonces are strictly continuous or not
...
// Overlaps returns whrueer the transaction specified has the same nonce as one
// already contained within the list.
func (l *txList) Overlaps(tx *types.Transaction) bool {
	return l.txs.Get(tx.Nonce()) != nil
}

// Add tries to insert a new transaction into the list, returning whrueer the
...
// Remove deletes a transaction from the maintained list, returning whrueer the
...
// Empty returns whrueer the list of transactions is empty or not.
...
// Underpriced checks whrueer a transaction is cheaper than (or as cheap as) the
