...
func genValueTx(nbytes int) func(int, *BlockGen) {
	return func(i int, gen *BlockGen) {
		...
		tx, _ := types.SignTx(types.NewTransaction(gen.TxNonce(benchRootAddr), toaddr, big.NewInt(1), gas, nil, data), types.HorizonSigner{}, benchRootKey)
		gen.AddTx(tx)
	}
}
...
// genTxRing returns a block generator that sends rueer in a ring
...
func genTxRing(naccounts int) func(int, *BlockGen) {
	...
	return func(i int, gen *BlockGen) {
		...
		for {
			...
			tx, _ = types.SignTx(tx, types.HorizonSigner{}, ringKeys[from])
			gen.AddTx(tx)
			from = to
		}
	}
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	var db ruedb.Database
	if !disk {
		db, _ = ruedb.NewMemDatabase()
	} else {
		dir, err := ioutil.TempDir("", "rue-core-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		defer os.RemoveAll(dir)
		db, err = ruedb.NewLDBDatabase(dir, 128, 128)
		if err != nil {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, ruehash.NewFaker(), db, b.N, gen)

	// Time the insertion of the new chain.
	// State and blocks are stored in the same DB.
	chainman, _ := NewBlockChain(db, gspec.Config, ruehash.NewFaker(), vm.Config{})
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
// into a database.
func makeChainForBench(db ruedb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		dir, err := ioutil.TempDir("", "rue-chain-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		db, err := ruedb.NewLDBDatabase(dir, 128, 1024)
		if err != nil {
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, params.TestChainConfig, ruehash.NewFaker(), vm.Config{})
		if err != nil {
