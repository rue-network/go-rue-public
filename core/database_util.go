...
var (
	...
	configPrefix   = []byte("rueereum-config-") // config prefix for the db
...
func WriteCanonicalHash(db ruedb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db ruedb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db ruedb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db ruedb.Putter, hash common.Hash) error {
...
func WriteHeader(db ruedb.Putter, header *types.Header) error {
...
func WriteBody(db ruedb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db ruedb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db ruedb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db ruedb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db ruedb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db ruedb.Putter, block *types.Block) error {
...
func WriteBloomBits(db ruedb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db ruedb.Database) ruedb.Database {
	return ruedb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db ruedb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db ruedb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db ruedb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
