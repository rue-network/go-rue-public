...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (ruedb.Database, error) {
	if ctx.config.DataDir == "" {
		return ruedb.NewMemDatabase()
	}
	db, err := ruedb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
