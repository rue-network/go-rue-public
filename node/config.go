...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of grue is "grue". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Grue", keep that.
	if name == "grue" || name == "grue-testnet" {
		name = "Grue"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "grue" instances.
var isOldGrueResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by grue 1.4 are used if they exist.
	if c.name() == "grue" && isOldGrueResource[path] {
		oldpath := ""
		if c.Name == "grue" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-rueereum-keystore")
		ephemeral = keydir
	}
