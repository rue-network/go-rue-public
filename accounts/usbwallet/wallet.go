...
type wallet struct {
	...
	deriveChain    rueereum.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain rueereum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
