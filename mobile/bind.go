...
func (s *signer) Sign(addr *Address, unsignedTx *Transaction) (signedTx *Transaction, _ error) {
	sig, err := s.sign(types.HorizonSigner{}, addr.address, unsignedTx.tx)
	if err != nil {
		return nil, err
	}
...
func DeployContract(opts *TransactOpts, abiJSON string, bytecode []byte, client *RuereumClient, args *Interfaces) (contract *BoundContract, _ error) {
...
func BindContract(address *Address, abiJSON string, client *RuereumClient) (contract *BoundContract, _ error) {
	parsed, err := abi.JSON(strings.NewReader(abiJSON))
	if err != nil {
		return nil, err
	}
