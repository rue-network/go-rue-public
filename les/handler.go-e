...
const (
	...
	rueVersion = 63 // equivalentrueversion for the downloader
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case GetHeaderProofsMsg:
		...
		trieDb := ruedb.NewTable(pm.chainDb, light.ChtTablePrefix)
		for _, req := range req.Reqs {
			if bytes >= softResponseLimit {
				break
			}
	...
	case GetHelperTrieProofsMsg:
		...
		for _, req := range req.Reqs {
			...
			if tr == nil || req.HelperTrieType != lastType || req.TrieIdx != lastIdx {
				...
				if root != (common.Hash{}) {
					if t, err := trie.New(root, ruedb.NewTable(pm.chainDb, prefix)); err == nil {
						tr = t
					}
				}
...
func (d *downloaderPeerNotify) registerPeer(p *peer) {
	...
	pm.downloader.RegisterLightPeer(p.id, rueVersion, pc)
}
